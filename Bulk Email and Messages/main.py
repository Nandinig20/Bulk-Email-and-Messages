...
def button_check():
    if choice.get() == 'multiple':
        browserButton.config(state=NORMAL)
        toEntryField.config(state='readonly')

    if choice.get() == 'single':
        browserButton.config(state=DISABLED)
        toEntryField.config(state=NORMAL)
    ...
    if toEntryField.get()=='' or subjectEntryField.get()=='' or textarea.get(1.0,END)=='\n':
        ...
        if choice.get()=='multiple':
            ...
            if result=='failed':
                ...
                sendLabel.config(text='Sent:' + str(sent))
            ...
            messagebox.showinfo('Success','Email are sent successfully')
...
def settings():
    ...
    def save():
      if fromEntryField.get()=='' or passwordEntryField.get=='':
          messagebox.showerror('Error', 'All Feilds Are Requried', parent=root1)
          ...
          f.close()
          messagebox.showinfo('Information','CREDENTIAL SAVED SUCCESSFULLY', parent=root1)
    ...
    Label(root1, text='Credential Setting',image=logoImage, compound=LEFT, font=('goudy old style', 40,'bold'), fg='white', bg='gray20').grid(padx=60)
...
singleRadioButton=Radiobutton(chooseFrame,text='Single', font=('times new roman', 25, 'bold'), variable=choice, value='single', bg='dodger blue2',activebackground='dodger blue2',command=button_check)
singleRadioButton.grid(row=0, column=0, padx=20)

multipleRadioButton=Radiobutton(chooseFrame,text='Multiple', font=('times new roman', 25, 'bold'), variable=choice, value='multiple', bg='dodger blue2',activebackground='dodger blue2',command=button_check)
...
browserButton=Button(toLabelFrame, text='Browser', image=browserImage, compound=LEFT, font=('arial', 12, 'bold'), cursor='hand2', bd=0, bg='dodger blue2',activebackground='dodger blue2', state=DISABLED,command=browse)
browserButton.grid(row=0, column=1, padx=20)
...
Button(emailLabelFrame, text='Speak', image=micImage, compound=LEFT, font=('arial', 12, 'bold'), cursor='hand2', bd=0, bg='dodger blue2',activebackground='dodger blue2', command=speak).grid(row=0, column=0)

attachImage=PhotoImage(file='attachments.png')

Button(emailLabelFrame, text='Attachment', image=attachImage, compound=LEFT, font=('arial', 12, 'bold'), cursor='hand2', bd=0, bg='dodger blue2',activebackground='dodger blue2', command=attachment).grid(row=0, column=1)
...
Button(root,image=sendImage, bd=0,bg='dodger blue2',cursor='hand2',activebackground='dodger blue2', command=send_email).place(x=490, y=540)
...
Button(root,image=exitImage, bd=0,bg='dodger blue2',cursor='hand2',activebackground='dodger blue2',command=iexist).place(x=690, y=550)
